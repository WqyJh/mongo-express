...
window.checkValidJSON = function () {
  $.ajax({
    type: 'POST',
    url: `${ME_SETTINGS.ajaxBaseHref}checkValid`,
    data: {
...
function makeCollectionUrl() {
  ...
  return `${st.ajaxBaseHref}db/${encodeURIComponent(st.dbName)}/${encodeURIComponent(st.collectionName)}/`;
}
...
$(() => {
  ...
  $importInputsFile.on('change', function (event) {
    ...
    $.ajax({
      type: 'POST',
      url: `${ME_SETTINGS.ajaxBaseHref}db/${ME_SETTINGS.dbName}/import/${collection}`,
      data,
      cache: false,
